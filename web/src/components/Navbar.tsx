...
const Navbar: React.FC<NavbarProps> = ({ onToggle }) => {
  ...
  return (
    <div
      className={`h-screen bg-blue-900 text-white fixed top-0 left-0 flex flex-col transition-all duration-300 
        ${isOpen ? "w-56" : "w-16"}`}
    >
      {/* Header */}
        ...
        <nav className="flex flex-col p-3 space-y-2">
          ...
          {openLessons && isOpen && (
            <div className="ml-6 space-y-1 text-sm">
              <Link to="/lessons/language" className="block hover:text-blue-300">
                🌐 Ngôn ngữ
              </Link>
              <Link to="/lessons/emotion" className="block hover:text-blue-300">
                😊 Cảm xúc
              </Link>
              <Link to="/lessons/focus" className="block hover:text-blue-300">
                🧩 Tập trung
              </Link>
              <Link to="/lessons/math" className="block hover:text-blue-300">
                🔢 Toán
              </Link>
              <Link to="/lessons/life-skills" className="block hover:text-blue-300">
                🌱 Kỹ năng sống
              </Link>
            </div>
          )}
          ...
          {openGames && isOpen && (
            <div className="ml-6 space-y-1 text-sm">
              <Link to="/games/memory" className="block hover:text-blue-300">
                🧠 Trí nhớ
              </Link>
              <Link to="/games/active" className="block hover:text-blue-300">
                🏃🏻‍➡️ Vận động
              </Link>
              <Link to="/games/puzzle" className="block hover:text-blue-300">
                ✏️ Ghép hình & Tô màu
              </Link>
              <Link to="/games/co-op" className="block hover:text-blue-300">
                🧑‍🤝‍🧑 Hợp tác
              </Link>
            </div>
          )}
          ...
          {openGroups && isOpen && (
            <div className="ml-6 space-y-1 text-sm">
              <Link to="/groups/create" className="block hover:text-blue-300">
                ➕ Tạo nhóm
              </Link>
              <Link to="/groups/join" className="block hover:text-blue-300">
                👋 Tham gia nhóm
              </Link>
              <Link to="/groups/tasks" className="block hover:text-blue-300">
                📖 Nhiệm vụ chung
              </Link>
            </div>
          )}
          ...
          {openRewards && isOpen && (
            <div className="ml-6 space-y-1 text-sm">
              <Link to="/rewards/badges" className="block hover:text-blue-300">
                🥇 Huy hiệu
              </Link>
              <Link to="/rewards/characters" className="block hover:text-blue-300">
                😀 Sticker & Nhân vật
              </Link>
              <Link to="/rewards/collection" className="block hover:text-blue-300">
                🎒 Bộ sưu tập
              </Link>
            </div>
          )}
          ...
          <Link to="/admin/lesson-group" className="px-3 py-2 rounded-md hover:bg-blue-600 transition">
            📂 {isOpen && "Nhóm bài học"}
          </Link>
          <Link to="/admin/lesson" className="px-3 py-2 rounded-md hover:bg-blue-600 transition">
            📘 {isOpen && "Bài học"}
          </Link>
